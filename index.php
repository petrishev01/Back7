...
if(isset($_SESSION['user']) && $_SERVER['REQUEST_METHOD'] == 'GET' ){ //GET запрос с залогининым пользователем
    ...
    $value['name'] = empty($_COOKIE['name_value']) ? '' :  strip_tags($_COOKIE['name_value']);
    $value['email'] = empty($_COOKIE['email_value']) ? '' :  strip_tags($_COOKIE['email_value']);
    $value['bio'] = empty($_COOKIE['bio_value']) ? '' :  strip_tags($_COOKIE['bio_value']);
    $value['year'] = empty($_COOKIE['year_value']) ? '' :  strip_tags($_COOKIE['year_value']);
    $value['gender'] = empty($_COOKIE['gender_value']) ? '' :  strip_tags($_COOKIE['gender_value']);
    $value['limbs'] = empty($_COOKIE['limbs_value']) ? '' :  strip_tags($_COOKIE['limbs_value']);
